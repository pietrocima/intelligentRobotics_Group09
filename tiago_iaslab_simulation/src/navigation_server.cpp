...
public:
    ...
    void executeCB(const tiago_iaslab_simulation::MoveTiagoGoalConstPtr &goal){
        ...
        myQuaternion.setRPY(0,0,goal->yaw);  //insert roll, pitch and yaw from the input.
        ...
        //Define waypoint
        move_base_msgs::MoveBaseGoal goalWayPoint;
        goalWayPoint.target_pose.header.frame_id = "map";
        goalWayPoint.target_pose.header.stamp = ros::Time::now();

        goalWayPoint.target_pose.pose.position.x = 8.0;
        goalWayPoint.target_pose.pose.position.y = 0.0;
        //Define the orientation of the waypoint
        goalWayPoint.target_pose.pose.orientation.w = 1.0;

        ac_in.sendGoal(goalWayPoint);
        ac_in.waitForResult();
        ...
        goalMove.target_pose.pose.orientation.w = myQuaternion.getW();        
        goalMove.target_pose.pose.orientation.x = myQuaternion.getX();
        goalMove.target_pose.pose.orientation.y = myQuaternion.getY();
        goalMove.target_pose.pose.orientation.z = myQuaternion.getZ();
        ...
        if(ac_in.getState() == actionlib::SimpleClientGoalState::SUCCEEDED){
            ...
            listener.waitForTransform("base_laser_link","map",cloud_laserFrame.header.stamp,ros::Duration(5.0));
            listener.transformPointCloud("map",cloud_laserFrame,cloud_mapFrame);
            ...
            for(int i =0; i < boolObs.size(); i++){
                ...
                if( (i < boolObs.size()) && boolObs.at(i) == 1){
                    ...
                    if (endObs - startObs > obstacleWidth){
                        ...
                        result_.sequence.push_back(mid_x_obs);
                        result_.sequence.push_back(mid_y_obs);
                    }
                }  
            }
            }
